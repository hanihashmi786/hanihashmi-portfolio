...
<template>
  ...
  <div class="bg-th-card px-5 py-5 md:px-12 md:py-10 text-left border border-th-border rounded-3xl text-th-fg mx-3 mb-5">
    <article>
      <header>
        <div class="text-2xl font-bold text-th-fg mb-5 fadein-bot title-section flex items-center">
          About Me &nbsp;
          <div class="h-[1px] w-32 bg-th-fg md:w-96"></div>
        </div>
      </header>
      <section class="flex flex-col md:flex-row gap-6 md:gap-10 items-start">
        <!-- Profile Image -->
        <div class="flex-shrink-0 mx-auto md:mx-0">
          <div class="w-40 h-40 md:w-48 md:h-48 rounded-2xl overflow-hidden border-2 border-th-border">
            <img src="/images/profile.png" alt="Hani Hashmi" class="w-full h-full object-cover" />
          </div>
        </div>
        <!-- Text -->
        <div class="flex flex-col gap-4 text-sm md:text-base text-justify">
          <p class="leading-relaxed fadein-left fadeins-1 text-th-muted">
          ...
          <p class="leading-relaxed fadein-left fadeins-2 text-th-muted">
  ...
  <div class="px-5 py-5 md:px-12 md:py-10 text-left text-th-fg mx-3 mb-5">
    <article>
      <header>
        <div class="text-2xl font-bold text-th-fg mb-8 fadein-bot title-section flex items-center">
          <div class="h-[1px] w-10 bg-th-fg md:w-20"></div>
          &nbsp; Work Experience
        </div>
      </header>
      <section>
        <div class="relative">
          <!-- Timeline line -->
          <div class="absolute left-4 top-0 bottom-0 w-[2px] bg-th-border hidden md:block"></div>
          <div v-for="(exp, index) in experiences" :key="index" class="mb-8 md:ml-12 relative">
            <!-- Timeline dot -->
            <div class="absolute -left-[3.25rem] top-1 w-3 h-3 rounded-full bg-th-fg hidden md:block" :class="{ 'ring-4 ring-th-subtle/30': exp.current }"></div>
            <div class="bg-th-card border border-th-border rounded-xl p-5 md:p-6 hover:bg-th-card-hover transition-colors duration-300">
              <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-3">
                <div>
                  <h3 class="text-lg font-semibold text-th-fg">{{ exp.role }}</h3>
                  <p class="text-th-accent text-sm">{{ exp.company }}</p>
                </div>
                <div class="text-right mt-2 md:mt-0">
                  <span v-if="exp.current" class="inline-block px-2 py-1 text-xs bg-th-fg/10 text-th-fg rounded-full mb-1">Current</span>
                  <p class="text-th-muted text-xs">{{ exp.period }}</p>
                  <p class="text-th-subtle text-xs">{{ exp.location }}</p>
                </div>
              </div>
              <ul class="list-disc list-inside text-th-muted text-sm leading-relaxed flex flex-col gap-2">
  ...
  <div class="px-5 py-5 md:px-12 md:py-10 text-left text-th-fg mx-3 mb-5">
    <article>
      ...
      <section>
        <div v-for="(edu, index) in education" :key="index" class="bg-th-card border border-th-border rounded-xl p-5 md:p-6 mb-4">
          <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
            <div>
              <h3 class="text-lg font-semibold text-th-fg">{{ edu.degree }}</h3>
              <p class="text-th-accent text-sm">{{ edu.institution }}</p>
            </div>
            <div class="text-right mt-2 md:mt-0">
              <p class="text-th-muted text-xs">{{ edu.period }}</p>
              <p class="text-th-subtle text-xs">{{ edu.location }}</p>
            </div>
          </div>
          <div class="mt-3">
            <h4 class="text-sm font-medium text-th-accent mb-2">Key Highlights</h4>
            <ul class="list-disc list-inside text-th-muted text-sm leading-relaxed flex flex-col gap-2">
  ...
  <div class="px-5 py-5 md:px-12 md:py-10 text-left text-th-fg mx-3 mb-5">
    <article>
      ...
      <section>
        <div>
          <ul class="flex flex-wrap text-sm font-medium text-center text-th-subtle mb-5">
            <li class="mr-2">
              <button class="inline-block px-4 py-3 rounded-lg hover:text-th-fg transition-colors"
                :class="{ 'text-th-fg bg-th-fg/10': activeTab === 1 }" @click="activeTab = 1">Tech Stack</button>
            </li>
            <li class="mr-2">
              <button class="inline-block px-4 py-3 rounded-lg hover:text-th-fg transition-colors"
                :class="{ 'text-th-fg bg-th-fg/10': activeTab === 2 }" @click="activeTab = 2">Tools</button>
            </li>
          </ul>
        </div>
        <div v-show="activeTab === 1">
          <div class="grid grid-cols-2 gap-4 md:grid-cols-3 md:gap-8 xl:grid-cols-4 xl:gap-10 2xl:gap-12">
            <div v-for="item in tech" :key="item.id">
              <div class="item-tech flex cursor-pointer items-center gap-2 rounded border border-th-border px-2 py-2 hover:bg-th-card-hover md:gap-3 lg:px-3 transition-colors">
                ...
                <div class="flex items-center text-sm md:text-base lg:text-lg">
                  <div class="tech font-medium text-th-fg transition-all duration-300 translate-y-0">{{ item.name }}</div>
                  <div class="status-tech opacity-0 absolute mt-5 text-[10px] text-th-accent transition-all duration-300 md:text-xs lg:text-sm">{{ item.status }}</div>
                </div>
              </div>
            </div>
          </div>
  ...
  <div class="px-5 py-5 md:px-12 md:py-10 text-left text-th-fg mx-3 mb-32">
    <article>
      <header>
        <div class="text-2xl font-bold text-th-fg mb-8 fadein-bot title-section flex items-center">
          Certifications &nbsp;
      ...
      <section>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
          <component
            ...
            :rel="cert.url ? 'noopener' : undefined"
            class="bg-th-card border border-th-border rounded-xl p-5 hover:bg-th-card-hover transition-all duration-300 hover:-translate-y-1"
            :class="{ 'cursor-pointer': cert.url }">
            <div class="flex items-start gap-3">
              <div class="w-10 h-10 rounded-lg bg-th-fg/10 flex items-center justify-center flex-shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-th-fg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" />
                </svg>
              </div>
              <div>
                <h3 class="text-sm font-semibold text-th-fg">{{ cert.name }}</h3>
                <p class="text-th-accent text-xs mt-1">{{ cert.issuer }}</p>
                <div class="flex items-center gap-2 mt-2">
                  <span class="inline-block px-2 py-0.5 text-[10px] bg-th-fg/10 text-th-muted rounded-full">{{ cert.category }}</span>
                  <span v-if="cert.url" class="text-[10px] text-th-accent flex items-center gap-1">
